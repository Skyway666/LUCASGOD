...
int timer(int width, int combo, int slowdown)
{
	if (combo == 1 && slowdown % 10 == 0)
	{
		width++;
	}
...
bool collide(int x, int y, int w, int h, struct SDL_Rect rectangle)
{
	if (y + h > rectangle.y && x + w > rectangle.x && rectangle.y + rectangle.h > y && rectangle.x + rectangle.w > x)
	{
		return true;
	}
...
void Draw()
{
	...
	for(int i = 0; i < NUM_SHOTS; ++i)
	{
		...
		if (collide(g.shots[i].x, g.shots[i].y, 64, 64, rectangle1))
	{
		l = 0;
	}
